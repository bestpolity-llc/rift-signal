...
# Use the complete cleaned frame, including its finished name label.
from PIL import ImageOps
clean = Path.home() / "Downloads/captain-marcus-vale-clean.png"
with Image.open(clean) as original:
    artwork = ImageOps.contain(
        original.convert("RGB"), (1280,720),
        method=Image.Resampling.LANCZOS
    )
    canvas = Image.new("RGB",(1280,720),"#061019")
    canvas.paste(artwork,(
        (1280-artwork.width)//2,
        (720-artwork.height)//2
    ))
    canvas.save(OUT/"dvd/menus/captain.png")
...
print("Captain screen uses a tighter head-and-shoulders crop.")
